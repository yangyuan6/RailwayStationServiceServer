...
<div id="dialog" class="none">
    <div id="dialog-wrapper">
        <div class="dialog-header">
            <span>Тэргэн билетийн мэдээлэл</span>
        </div>
        <div class="dialog-content">
            <c:choose>
                <c:when test="${ticketMsg.state== 2}">

                    <p>Та ${ticketMsg.ticketData.trainNumber} дүгээр галт  тэргэнд суухын тулд</p>
                    <p><b>${dataMap["witingRoom"]} </b></p>
                     <p><b>${dataMap["station"]} хаалгаар орж , </b> зогсоолд очно уу. Галт тэрэг хөдлөхөд</p>
                    <p><b>${dataMap['distanceTime']}</b></p>
                    <audio  id="audio">
                        <source src="${audioUrl}/RailwayStationServiceVoice/mo/${voice}.wav"  type="audio/mpeg">
                    </audio>
                </c:when>
                <c:otherwise>
                    <p>${ticketMsg.msg}</p>
        ...
        <div class="dialog-footer">
            <c:choose>
                <c:when test="${ticketMsg.state== 2}">
                    <button id="btn_play" onclick="playVoice()">Нэвтрүүлэх</button>
                </c:when>
            </c:choose>
            <button id="btn_close" onclick="toTagetUrl(${ticketMsg.state},'${ticketMsg.url}')">Хаах</button>
        </div>
    </div>
</div>
